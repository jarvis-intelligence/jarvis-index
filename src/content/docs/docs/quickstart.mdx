...
# Codex CLI
codex plugin marketplace add jarvis-intelligence/jarvis-index
```

For Cursor: open Settings → Plugins → search "jarvis" → Install.
...
Requires `uv` on PATH. This is also the fix for the uvx cold-start timeout you'd otherwise hit
...
The `--python 3.13` pin is deliberate: on a machine with no Python at all, `uv tool install`
...
::::tip[Optional extras]
- `uv tool install --python 3.13 "jarvis-mcp[semantic]"` — enables `semanticSearch` (vector + lexical fusion)
- `uv tool install --python 3.13 "jarvis-mcp[watch]"` — enables `jarvis watch` (auto-reindex on file changes)
::::
...
This is a real cold-install run recorded on a fresh, isolated environment (fresh `HOME`,
...
import ColdInstallPlayer from '../../../components/landing/ColdInstallPlayer.astro';
